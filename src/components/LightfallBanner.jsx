...
export default function LightfallBanner({
  ...
  useEffect(() => {
    ...
    /* Try webgl2 first; if it fails, swap the canvas for a fresh one and try webgl1.
    ...
    function resize() {
      ...
      const w = Math.max(1, Math.round(rect.width  * dprVal));
      const h = Math.max(1, Math.round(rect.height * dprVal));
    ...
    const io = new IntersectionObserver(([e]) => { inView = e?.isIntersecting ?? true; }, { threshold: 0 });
    ...
    function loop(t) {
      ...
      if (!inView || !visible) { lastT = t; return; }
