...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    /* skip the WebGL beam entirely on tiny viewports or when reduced-motion is requested */
    const reduceMot = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
    if (reduceMot || window.innerWidth < 640) return;
    ...
    baseDprRef.current = Math.min(dpr ?? (window.devicePixelRatio || 1), window.innerWidth < 1024 ? 1.25 : 2);
    ...
    const updateMouse = (clientX, clientY) => {
      ...
      mouseTarget.set((clientX - rect.left) * ratio, (rect.height - (clientY - rect.top)) * ratio);
    };
    ...
    const animate = () => {
      ...
      if (now - lastFpsCheckRef.current > 750 && fpsSamplesRef.current.length > 0) {
        ...
        if (avg < 50) next = clamp(currentDprRef.current * 0.85, 0.6, base);
        else if (avg > 58 && currentDprRef.current < base) next = clamp(currentDprRef.current * 1.1, 0.6, base);
        if (Math.abs(next - currentDprRef.current) > 0.01 && now - lastDprChangeRef > 2000) {
