...
export default function SplashCursor({
  ...
  const [enabled, setEnabled] = useState(() => {
    ...
    const reduceMot = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
    return !(isCoarse || reduceMot || window.innerWidth < 768);
  });
  ...
  useEffect(() => {
    ...
    function updateKeywords() {
      let k = []; if (config.SHADING) k.push('SHADING'); displayMaterial.setKeywords(k);
    }
    ...
    function calcDeltaTime() {
      let now = Date.now(), dt = Math.min((now-lastUpdateTime)/1000, 0.016666); lastUpdateTime=now; return dt;
    }
    ...
    function step(dt) {
      ...
      gl.uniform1i(vorticityProgram.uniforms.uVelocity,velocity.read.attach(0)); gl.uniform1i(vorticityProgram.uniforms.uCurl,curl.attach(1));
      gl.uniform1f(vorticityProgram.uniforms.curl,config.CURL); gl.uniform1f(vorticityProgram.uniforms.dt,dt); blit(velocity.write); velocity.swap();
      divergenceProgram.bind(); gl.uniform2f(divergenceProgram.uniforms.texelSize,velocity.texelSizeX,velocity.texelSizeY);
      gl.uniform1i(divergenceProgram.uniforms.uVelocity,velocity.read.attach(0)); blit(divergence);
      clearProgram.bind(); gl.uniform1i(clearProgram.uniforms.uTexture,pressure.read.attach(0));
      gl.uniform1f(clearProgram.uniforms.value,config.PRESSURE); blit(pressure.write); pressure.swap();
      pressureProgram.bind(); gl.uniform2f(pressureProgram.uniforms.texelSize,velocity.texelSizeX,velocity.texelSizeY);
      ...
      gl.uniform1i(gradienSubtractProgram.uniforms.uPressure,pressure.read.attach(0)); gl.uniform1i(gradienSubtractProgram.uniforms.uVelocity,velocity.read.attach(1));
      ...
      let velocityId=velocity.read.attach(0); gl.uniform1i(advectionProgram.uniforms.uVelocity,velocityId);
      ...
      gl.uniform1i(advectionProgram.uniforms.uVelocity,velocity.read.attach(0)); gl.uniform1i(advectionProgram.uniforms.uSource,dye.read.attach(1));
    ...
    function splat(x,y,dx,dy,color) {
      ...
      gl.uniform2f(splatProgram.uniforms.point,x,y); gl.uniform3f(splatProgram.uniforms.color,dx,dy,0);
    ...
    function updatePointerDownData(p,id,x,y) {
      p.id=id; p.down=true; p.moved=false; p.texcoordX=x/canvas.width; p.texcoordY=1-y/canvas.height;
    ...
    function updatePointerMoveData(p,x,y,color) {
      ...
      p.deltaX=correctDeltaX(p.texcoordX-p.prevTexcoordX); p.deltaY=correctDeltaY(p.texcoordY-p.prevTexcoordY);
    ...
    const BRAND_COLORS = [
      {r:0.220,g:0.741,b:0.973},{r:0.157,g:0.647,b:0.878},{r:0.000,g:0.788,b:0.694},
      {r:0.380,g:0.560,b:1.000},{r:0.490,g:0.310,b:1.000},
    ];
    ...
    function getResolution(res){let a=gl.drawingBufferWidth/gl.drawingBufferHeight;if(a<1)a=1/a;const min=Math.round(res),max=Math.round(res*a);return gl.drawingBufferWidth>gl.drawingBufferHeight?{width:max,height:min}:{width:min,height:max};}
    ...
    function handleMouseDown(e) {
      let p=pointers[0]; updatePointerDownData(p,-1,scaleByPixelRatio(e.clientX),scaleByPixelRatio(e.clientY)); clickSplat(p);
    }
    let firstMove=false;
    function handleMouseMove(e) {
      let p=pointers[0],x=scaleByPixelRatio(e.clientX),y=scaleByPixelRatio(e.clientY);
      let color=firstMove?p.color:generateColor(); firstMove=true; updatePointerMoveData(p,x,y,color);
    }
    function handleTouchStart(e) {
      const t=e.targetTouches,p=pointers[0];
      for(let i=0;i<t.length;i++) updatePointerDownData(p,t[i].identifier,scaleByPixelRatio(t[i].clientX),scaleByPixelRatio(t[i].clientY));
    }
    function handleTouchMove(e) {
      const t=e.targetTouches,p=pointers[0];
      for(let i=0;i<t.length;i++) updatePointerMoveData(p,scaleByPixelRatio(t[i].clientX),scaleByPixelRatio(t[i].clientY),p.color);
    }
    function handleTouchEnd(e) {
      const t=e.changedTouches,p=pointers[0]; for(let i=0;i<t.length;i++) updatePointerUpData(p);
    }
  ...
  useEffect(() => {
    const onResize = () => {
      const isCoarse  = window.matchMedia?.('(pointer: coarse)').matches;
      const reduceMot = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
      setEnabled(!(isCoarse || reduceMot || window.innerWidth < 768));
    };
