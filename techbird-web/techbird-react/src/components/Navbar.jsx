...
export default function Navbar() {
  ...
  const toggleAccordion = (name) => setMobileAccordion(prev => prev === name ? null : name);
    ...
    <nav className="topbar" id="topbar">
      ...
      <ul className="nav-links">
        ...
        <li onMouseEnter={() => open('svc')} onMouseLeave={close}>
          <Link to="/services" className={`nav-trigger${mega === 'svc' ? ' active' : ''}`}>
            Services <Chev />
          </Link>
        </li>
      ...
      <div className={`mobile-drawer${mobileOpen ? ' is-open' : ''}`}>
        ...
        <ul className="mobile-nav-list">
          <li><Link to="/" className="mobile-nav-link" onClick={closeMobile}>Home</Link></li>

          {/* Services accordion */}
          <li>
            <button
              className={`mobile-nav-link mobile-nav-accordion${mobileAccordion === 'svc' ? ' is-open' : ''}`}
              onClick={() => toggleAccordion('svc')}
            >
              Services
              <svg className="mobile-chev" viewBox="0 0 10 6" fill="none">
                <path d="M1 1l4 4 4-4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
            {mobileAccordion === 'svc' && (
              <ul className="mobile-sub-list">
                {SERVICES.map(s => (
                  <li key={s.to}>
                    <Link to={s.to} className="mobile-sub-link" onClick={closeMobile}>
                      <span className="mobile-sub-icon" style={{ background: s.color + '18', color: s.color }}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
                          <path d={s.icon}/>
                        </svg>
                      </span>
                      <span>
                        <span className="mobile-sub-name">{s.label}</span>
                        <span className="mobile-sub-desc">{s.desc}</span>
                      </span>
                    </Link>
                  </li>
                ))}
          ...
          <li>
            <button
              className={`mobile-nav-link mobile-nav-accordion${mobileAccordion === 'pro' ? ' is-open' : ''}`}
              onClick={() => toggleAccordion('pro')}
            >
              Products
              <svg className="mobile-chev" viewBox="0 0 10 6" fill="none">
                <path d="M1 1l4 4 4-4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
            ...
            {mobileAccordion === 'pro' && (
              <ul className="mobile-sub-list">
                {PRODUCTS.map(p => (
                  <li key={p.label}>
                    <Link to={p.to} className="mobile-sub-link" onClick={closeMobile}>
                      <span className="mobile-sub-icon" style={{ background: p.color + '18', color: p.color }}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
                          <path d={p.icon}/>
                        </svg>
                      </span>
                      <span className="mobile-sub-name">{p.label}</span>
                    </Link>
                  </li>
                ))}
              </ul>
          ...
          <li><Link to="/#about" className="mobile-nav-link" onClick={closeMobile}>About</Link></li>
          <li><Link to="/#blog" className="mobile-nav-link" onClick={closeMobile}>Blog</Link></li>
          <li><Link to="/#contact" className="mobile-nav-link" onClick={closeMobile}>Contact</Link></li>
        </ul>

        <div className="mobile-drawer-foot">
          <Link to="/#contact" className="btn-pill" onClick={closeMobile}>
            <span>Get Started</span>
            <i className="arrow"></i>
          </Link>
        </div>
